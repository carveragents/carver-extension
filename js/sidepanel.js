...
class RegulatoryMonitorSidePanel {
  ...
  setupEventListeners() {
    // API Key setup
    document
      .getElementById("validate-api-key")
      ?.addEventListener("click", () => this.handleApiKeyValidation());
    document
    ...
    document
      ...
      ?.addEventListener("click", () => this.toggleLoginApiKeyVisibility());

    // Tab Navigation
    document
      .getElementById("regulatory-tab")
      ?.addEventListener("click", () => this.switchTab("regulatory"));
    document
      .getElementById("horizon-tab")
      ?.addEventListener("click", () => this.switchTab("horizon"));

    // Navigation
    document
      .getElementById("back-btn")
      ?.addEventListener("click", () => this.showScreen("main-dashboard"));
    document
      .getElementById("search-back-btn")
      ?.addEventListener("click", () => this.showScreen("main-dashboard"));
    document
      .getElementById("subscriptions-back-btn")
      ?.addEventListener("click", () => this.returnToMainFromSubscriptions());
    document
      .getElementById("settings-back-btn")
      ?.addEventListener("click", () => this.showScreen("main-dashboard"));
    document
      .getElementById("partner-back-btn")
      ?.addEventListener("click", () => this.showScreen("main-dashboard"));

    // Partner Details Screen Actions
    document
      .getElementById("edit-partner-btn")
      ?.addEventListener("click", () => this.handleEditPartnerFromScreen());
    document
      .getElementById("delete-partner-btn")
      ?.addEventListener("click", () => this.handleDeletePartnerFromScreen());

    // Partner Watch Event Listeners
    document
      .getElementById("add-partner-btn")
      ?.addEventListener("click", () => this.showAddPartnerModal());
    document
      .getElementById("add-first-partner-btn")
      ?.addEventListener("click", () => this.showAddPartnerModal());
    document
      .getElementById("close-partner-modal")
      ?.addEventListener("click", () => this.hideAddPartnerModal());
    document
      .getElementById("cancel-partner-btn")
      ?.addEventListener("click", () => this.hideAddPartnerModal());
    document
      .getElementById("save-partner-btn")
      ?.addEventListener("click", () => this.handleSavePartner());
    document
      .getElementById("horizon-refresh-btn")
      ?.addEventListener("click", () => this.refreshPartnerData());

    // Edit Partner Modal Event Listeners
    document
      .getElementById("close-edit-partner-modal")
      ?.addEventListener("click", () => this.hideEditPartnerModal());
    document
      .getElementById("cancel-edit-partner-btn")
      ?.addEventListener("click", () => this.hideEditPartnerModal());
    document
      .getElementById("save-edit-partner-btn")
      ?.addEventListener("click", () => this.handleUpdatePartner());

    // Share Summary Modal Event Listeners
    document
      .getElementById("copy-share-btn")
      ?.addEventListener("click", () => this.copyShareSummary());
    document
      .getElementById("close-share-modal")
      ?.addEventListener("click", () => this.hideShareSummaryModal());

    // Topic Summary Share Button Event Listeners
    document
      .getElementById("share-topic-summary")
      ?.addEventListener("click", () => this.showTopicSummaryModal());
    document
      .getElementById("close-overall-topic-summary-modal")
      ?.addEventListener("click", () => this.hideTopicSummaryModal());
    document
      .getElementById("copy-overall-topic-summary-btn")
      ?.addEventListener("click", () => this.copyTopicSummary());

    // Partner Summary Share Button Event Listeners
    document
      .getElementById("share-partner-summary")
      ?.addEventListener("click", () => this.showPartnerSummaryModal());
    document
      .getElementById("close-overall-partner-summary-modal")
      ?.addEventListener("click", () => this.hidePartnerSummaryModal());
    document
      .getElementById("copy-overall-partner-summary-btn")
      ?.addEventListener("click", () => this.copyPartnerSummary());

    // Unsubscribe Confirmation Modal Event Listeners
    document
      .getElementById("close-unsubscribe-confirm-modal")
      ?.addEventListener("click", () => this.hideUnsubscribeConfirmModal());
    document
      .getElementById("cancel-unsubscribe-btn")
      ?.addEventListener("click", () => this.hideUnsubscribeConfirmModal());
    document
      .getElementById("confirm-unsubscribe-btn")
      ?.addEventListener("click", () => this.confirmUnsubscribe());

    // Delete Partner Confirmation Modal Event Listeners
    document
      .getElementById("close-delete-partner-confirm-modal")
      ?.addEventListener("click", () => this.hideDeletePartnerConfirmModal());
    document
      .getElementById("cancel-delete-partner-btn")
      ?.addEventListener("click", () => this.hideDeletePartnerConfirmModal());
    document
      .getElementById("confirm-delete-partner-btn")
      ?.addEventListener("click", () => this.confirmDeletePartnerFromModal());

    // Partner Search Event Listeners
    document
      .getElementById("partner-search-input")
      ?.addEventListener("input", (e) =>
        this.handlePartnerSearch(e.target.value)
      );
    document
      .getElementById("partner-search-input")
      ?.addEventListener("focus", () => this.showPartnerSuggestions());
    document
      .getElementById("partner-search-input")
      ?.addEventListener("blur", () =>
        setTimeout(() => this.hidePartnerSuggestions(), 200)
      );

    // Topics Search Event Listeners
    document
      .getElementById("topics-search-input")
      ?.addEventListener("input", (e) =>
        this.handleTopicsSearch(e.target.value)
      );
    document
      .getElementById("topics-search-input")
      ?.addEventListener("focus", () => this.handleTopicsSearchFocus());
    document
      .getElementById("topics-search-input")
      ?.addEventListener("blur", () =>
        setTimeout(() => this.hideTopicsSuggestions(), 200)
      );
    document
      .getElementById("search-topics-btn")
      ?.addEventListener("click", () => this.focusTopicsSearch());

    // Legacy search functionality (keep for compatibility)
    document
      .getElementById("search-btn")
      ?.addEventListener("click", () => this.showSearchScreen());
    ...
    document
      .getElementById("subscriptions-btn")
      ?.addEventListener("click", () => this.showSubscriptions());
    document
      .getElementById("refresh-btn")
      ?.addEventListener("click", () => this.refreshData());
    document
      .getElementById("settings-btn")
      ?.addEventListener("click", () => this.showScreen("settings-screen"));
    ...
    document
      .getElementById("close-support-modal")
      ?.addEventListener("click", () => this.hideSupportModal());
    document
      .getElementById("copy-support-email-btn")
      ?.addEventListener("click", () => this.copySupportEmail());

    // Make support email clickable to copy
    document
      .querySelector(".support-email")
      ?.addEventListener("click", () => this.copySupportEmail());
    ...
    document
      .getElementById("search-execute-btn")
      ?.addEventListener("click", () => this.handleSearchScreenSearch());
    document
    ...
    document
      .getElementById("partner-input")
      ?.addEventListener("keypress", (e) => {
        if (e.key === "Enter") this.handleSaveKeyword();
      });
    document
      .getElementById("frequency-select")
      ?.addEventListener("keypress", (e) => {
        if (e.key === "Enter") this.handleSaveKeyword();
      });
  }
  ...
  renderTopicSummaries(summaries) {
    ...
    container.innerHTML = sortedSummaries
      .map((topic) => {
            ...
            <div class="topic-card" data-topic-id="${topic.tag_id}">
                <div class="topic-header">
                    <div class="topic-name">
                        <div class="topic-color" style="background-color: ${
                          topic.color
                        }"></div>
                        <span class="topic-title" title="${this.escapeHtml(
                          topic.tag_name
                        )}">${this.escapeHtml(
          this.truncateText(topic.tag_name, 30)
        )}</span>
                    </div>
                    ${
                      isSubscribed
                    ...
                    <div class="topic-actions">
                        <button class="unsubscribe-tile-btn" 
                                ...
                                data-topic-name="${this.escapeHtml(
                                  topic.tag_name
                                )}"
                ...
                <div class="topic-meta">
                      <span>${topic.link_count} ${
          topic.link_count === 1 ? "source" : "sources"
        } 
                      ${
                        topic.last_updated
                          ? `, Updated ${this.formatActualDate(
                              topic.last_updated
                            )}</span>`
...
${this.escapeHtml(this.cleanSummaryText(topic.meta_summary))}
  ...
  async showTopicDetails(topicId) {
    try {
      ...
      const topicDetails = await this.apiCall(
        `/extension/topics/${topicId}/details`
      );
      ...
      if (isSubscribed) {
        ...
        document.getElementById("tag-meta-summary").innerHTML =
          this.formatMetaSummary(topicDetails.meta_summary);
        document.getElementById("tag-last-updated").textContent =
          topicDetails.last_updated
            ? `Last updated: ${this.formatDate(topicDetails.last_updated)}`
            : "No recent updates";
  ...
  async isUserSubscribedToTopic(topicId) {
    try {
      ...
      return subscriptionsArray.some(
        (sub) => sub.topic_id === topicId || sub.subscription_value === topicId
      );
    } catch (error) {
  ...
  renderFeedEntries(feedEntries) {
    ...
    if (!feedEntries || feedEntries.length === 0) {
      container.innerHTML =
        '<p class="no-summary">No recent updates available yet.</p>';
      return;
    }
      ...
      .map(
        (entry) => `
            <div class="regwatch-tile" data-entry-id="${entry.entry_id}">
                <div class="regwatch-tile-header">
                    <div class="regwatch-tile-title">
                        <a href="${
                          entry.link
                        }" target="_blank" rel="noopener noreferrer" title="${this.escapeHtml(
          entry.title
        )}">
                            ${this.escapeHtml(
                              this.truncateText(entry.title, 20)
                            )}
                        </a>
                    </div>
                    <div class="regwatch-tile-actions">
                        <button class="regwatch-action-btn" data-action="share_summary" data-entry-id="${
                          entry.entry_id
                        }" data-entry-url="${this.escapeHtml(
          entry.link
        )}" data-entry-title="${this.escapeHtml(
          entry.title
        )}" data-entry-summary="${this.escapeHtml(
          entry.one_line_summary || entry.content_preview || ""
        )}" data-entry-five-point="${this.escapeHtml(
          entry.five_point_summary || ""
                ...
                <div class="regwatch-tile-summary">${this.escapeHtml(
                  entry.one_line_summary ||
                    entry.content_preview ||
                    "No summary available"
                )}</div>
                ${
                  !this.isEpochDate(entry.published_date)
                    ? `<div class="regwatch-tile-date">Published on: ${this.formatActualDate(
  ...
  addActionButtonListeners(container) {
    const actionButtons = container.querySelectorAll(
      ".link-action[data-action], .link-action-icon[data-action], .regwatch-action-btn[data-action], .partnerwatch-action-btn[data-action]"
    );
  ...
  async handleEntryAction(action, entryId, url, actionButton) {
    ...
    const tileContainer = actionButton.closest(
      ".link-item, .regwatch-tile, .partnerwatch-tile"
    );
    let allButtons = [];
    if (tileContainer) {
      allButtons = tileContainer.querySelectorAll(
        ".link-action, .regwatch-action-btn, .partnerwatch-action-btn"
      );
    ...
    try {
      ...
      if (action === "share_summary") {
        ...
        const title =
          actionButton.getAttribute("data-entry-title") || "No title";
        const fivePointSummary = actionButton.getAttribute(
          "data-entry-five-point"
        );
        const basicSummary =
          actionButton.getAttribute("data-entry-summary") ||
          "No summary available";

        // Use five-point summary if available, otherwise fall back to basic summary
        const summary =
          fivePointSummary && fivePointSummary.trim()
            ? fivePointSummary
            : basicSummary;
        ...
        const result = await this.apiCall(
          `/extension/entries/${entryId}/actions/${action}`,
          {
            method: "POST",
          }
        );
    ...
    modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="action-result">${this.escapeHtml(
  ...
  async loadHorizonData() {
    try {
      ...
      const horizonData = {
        ...
        recent_meta_summaries: keywords.filter(
          (k) =>
            k.meta_summary &&
            k.meta_summary !== `No summaries available for '${k.keyword}'`
        ),
      };
    ...
    container.innerHTML = `
            ${keywords
              .map((keyword, index) => {
                ...
                const sentimentDot =
                  this.renderSentimentDot(aggregatedSentiment);

                // Get one-line summary
                const summary = keyword.meta_summary || "No summary available";
                const truncatedSummary = this.truncateText(
                  this.cleanSummaryText(summary),
                  150
                );

                return `
                    <div class="topic-card partner-card" data-keyword-id="${
                      keyword.keyword_id
                    }" data-kind="${keyword.kind}">
                        <div class="topic-header">
                            <div class="topic-name">
                                <span class="topic-title" title="${this.escapeHtml(
                                  keyword.keyword
                                )}">${this.escapeHtml(
                  this.truncateText(keyword.keyword, 30)
                )}</span>
                            </div>
                            <div class="partner-actions">
                                <button class="btn btn-small btn-secondary edit-partner" data-keyword-id="${
                                  keyword.keyword_id
                                }" data-keyword-name="${this.escapeHtml(
                  keyword.keyword
                  ...
                  keyword.frequency || "daily"
                                ...
                                <button class="btn btn-small btn-secondary delete-partner" data-keyword-id="${
                                  keyword.keyword_id
                                }" data-keyword-name="${this.escapeHtml(
                  keyword.keyword
                        ...
                        <div class="topic-meta">
<span>${keyword.sources_count || 0} sources${
                  keyword.last_updated
                    ? `, Updated ${this.formatActualDate(keyword.last_updated)}`
                    : ", No recent updates"
                }</span>
                        </div>
                        <div class="partner-sentiment">
                            <span>Sentiment: ${this.capitalize(
                              aggregatedSentiment
                            )}</span>
                        </div>
                        <div class="topic-summary">
    ...
    container.querySelectorAll(".partner-card").forEach((card) => {
      card.addEventListener("click", (e) => {
        ...
        if (
          !e.target.classList.contains("edit-partner") &&
  ...
  async updateSubscribeButton(topicId) {
    try {
      ...
      const subscriptions = await this.apiCall(
        `/core/users/${userId}/topics/subscriptions`
      );
      const subscriptionsArray = Array.isArray(subscriptions)
        ? subscriptions
        : [];
  ...
  getUserId() {
    ...
    if (!userId) {
      console.error(
        "No user ID found in user object. Current user object:",
        this.currentUser
      );
      console.error(
        "Available user properties:",
        Object.keys(this.currentUser)
      );
      return null;
    }
  ...
  async subscribeToTopic(topicId) {
    try {
      ...
      if (!userId) {
        this.showToast(
          "User UUID not available. Please check backend API configuration.",
          "error"
        );
        return;
      }
      ...
      this.showToast(
        "Failed to subscribe to topic. Please try again.",
        "error"
      );
    }
  }
  ...
  async unsubscribeFromTopic(topicId) {
    try {
      ...
      await this.apiCall(
        `/core/users/${userId}/topics/${topicId}/unsubscribe`,
        {
          method: "DELETE",
        }
      );
      ...
      this.showToast(
        "Failed to unsubscribe from topic. Please try again.",
        "error"
      );
    }
  }
  ...
  retry() {
    ...
    if (this.retryCount <= this.maxRetries) {
      ...
      this.showError(
        "Maximum retry attempts reached. Please refresh the extension."
      );
    }
  }
  ...
  initializeSettingsScreen() {
    ...
    if (this.currentUser && userNameElement && userEmailElement) {
      ...
      userEmailElement.textContent =
        this.currentUser.email || "No email available";
    }
  }
  ...
  formatMetaSummary(summaryText) {
    ...
    return `<div class="meta-summary-content">
            ${points
              .map(
                (point) =>
                  `<p class="summary-point">• ${this.escapeHtml(point)}</p>`
              )
              .join("")}
        </div>`;
  }
  ...
  renderSentiment(summary) {
    ...
    if (score !== undefined && score !== null) {
      ...
      if (!isNaN(scoreValue)) {
        ...
        scoreDisplay = `<span class="sentiment-score">${percentage.toFixed(
          0
        )}%</span>`;
      }
    }
  ...
  renderKeywordFeedEntries(feedEntries, container) {
    if (!feedEntries || feedEntries.length === 0) {
      container.innerHTML =
        '<p class="no-summary">No individual summaries available yet.</p>';
      return;
    }
        ...
        (entry) => `
            <div class="link-item" data-entry-id="${entry.entry_id}">
                <div class="link-title">
                    <a href="${
                      entry.link
                    }" target="_blank" rel="noopener noreferrer" class="link-title-text">
                ...
                <div class="link-summary-text">${this.escapeHtml(
                  entry.one_line_summary ||
                    entry.content_preview ||
                    "No summary available"
                )}</div>
                <div class="link-actions">
                    <button class="link-action" data-action="share_summary" data-entry-id="${
                      entry.entry_id
                    }" data-entry-url="${this.escapeHtml(
          entry.link
        )}" data-entry-title="${this.escapeHtml(
  ...
  async handleTriggerSearchForKeyword(keywordId) {
    try {
      ...
      const result = await this.apiCall(
        `/horizon/keywords/${keywordId}/search`,
        {
          method: "POST",
        }
      );
  ...
  async handleSummaryAction(action, summaryId, sourceUrl, actionButton) {
    ...
    try {
      const result = await this.apiCall(
        `/horizon/summaries/${summaryId}/actions/${action}`,
        {
          method: "POST",
        }
      );
  ...
  async showSearchScreen() {
    try {
      ...
      const recommendations = await this.apiCall(
        "/extension/topics/recommendations"
      );
        ...
        (sub) => `
            <div class="subscription-item">
                <div class="subscription-info">
                    <div class="subscription-name">${this.escapeHtml(
                      sub.name || sub.topic_name || sub.subscription_name
                    )}</div>
                    <div class="subscription-description">${this.escapeHtml(
                      sub.description || ""
                    )}</div>
                </div>
                <div class="subscription-actions">
                    <button class="btn btn-small btn-danger unsubscribe-btn" data-topic-id="${
                      sub.id
                    }">
                        Unsubscribe
                    </button>
  ...
  async unsubscribeFromTopicInList(topicId) {
    try {
      ...
      console.log(
        `Unsubscribing from topic ${topicId} for user ${userId} (from list)`
      );
      await this.apiCall(
        `/core/users/${userId}/topics/${topicId}/unsubscribe`,
        {
  ...
  async unsubscribe(subscriptionId) {
    ...
    console.log(
      "Legacy unsubscribe method called, this should be updated to use topic-based unsubscribe"
    );
  ...
  async handleTagSearch() {
    ...
    try {
      ...
      if (!query) {
        ...
        // Show search results
        results = await this.apiCall(
          `/extension/topics/search?query=${encodeURIComponent(query)}&limit=20`
        );
  ...
  renderSearchResults(results, isRecommendations = false) {
    ...
    // Add header
    const headerText = isRecommendations
      ? "Recommended Topics"
      : "Search Results";
    const headerHTML = `
            <div class="${
              isRecommendations ? "recommendations-header" : "search-header"
            }">
                <h3>${headerText}</h3>
                ${
                  isRecommendations
            ...
            <div class="search-result-item" data-topic-id="${topic.tag_id}">
                <div class="search-result-header">
                    <div class="search-result-title">
                        <div class="topic-color" style="background-color: ${
                          topic.color
                        }; width: 12px; height: 12px; border-radius: 50%; display: inline-block; margin-right: 8px;"></div>
                        ${this.escapeHtml(topic.tag_name)}
                    </div>
                    <button class="search-subscribe-btn" 
                            data-topic-id="${topic.tag_id}" 
                            data-topic-name="${this.escapeHtml(topic.tag_name)}"
                            style="background: none; border: none; cursor: pointer; font-size: 18px; padding: 4px; color: ${
                              isSubscribed ? "#10b981" : "#6b7280"
                            };"
                            title="${
                              isSubscribed
    ...
    const subscribeButtons = container.querySelectorAll(
      ".search-subscribe-btn"
    );
    console.log(
      "Found subscribe buttons in search results:",
      subscribeButtons.length
    );
  ...
  handleModalEscKey(event) {
    if (event.key === "Escape") {
      ...
      const partnerDetailsModal = document.getElementById(
  ...
  async handleSaveKeyword() {
    ...
    try {
      ...
      if (result.success !== false) {
        ...
        this.showToast(
          `Failed to add keyword: ${result.message || "Unknown error"}`,
          "error"
        );
      }
    } catch (error) {
  ...
  showPartnerSuggestions(partners = []) {
    ...
    suggestionsContainer.innerHTML = partners
      .map(
        (partner) => `
            <div class="partner-suggestion" data-partner-id="${
              partner.keyword_id
            }">
                <div class="partner-suggestion-name">${this.escapeHtml(
                  partner.keyword
                )}</div>
                <div class="partner-suggestion-description">${this.escapeHtml(
                  partner.kind || ""
                )}</div>
            </div>
        `
    ...
    // Add click handlers
    suggestionsContainer
      .querySelectorAll(".partner-suggestion")
      .forEach((item) => {
        item.addEventListener("click", (e) => {
          const partnerId = e.currentTarget.getAttribute("data-partner-id");
          const partnerName = e.currentTarget.querySelector(
  ...
  filterPartnersDisplay(selectedPartnerId = null) {
    ...
    if (selectedPartnerId) {
      partnersToShow = this.allPartners.filter(
        (partner) =>
    ...
    const filteredData = {
      ...
      summaries_today: 0,
      recent_meta_summaries: partnersToShow.filter(
        (k) =>
          k.meta_summary &&
          k.meta_summary !== `No summaries available for '${k.keyword}'`
      ),
  ...
  async showTopicsSuggestions(topics = []) {
    ...
    try {
      ...
      const subscriptionsArray = Array.isArray(subscriptions)
        ? subscriptions
        : [];
      subscribedTopicIds = subscriptionsArray.map(
        (sub) => sub.topic_id || sub.subscription_value
      );
    } catch (error) {
        ...
        return `
                <div class="topic-suggestion" data-topic-id="${topic.tag_id}">
                    <div class="topic-suggestion-content">
                        <div class="topic-suggestion-name">${this.escapeHtml(
                          topic.tag_name
                        )}</div>
                        <div class="topic-suggestion-description">${this.escapeHtml(
                          topic.description || ""
                        )}</div>
                    </div>
    ...
    suggestionsContainer
      .querySelectorAll(".topic-suggestion")
      .forEach((item) => {
        item.addEventListener("click", (e) => {
          ...
          // Check if user is subscribed to this topic
          const topicId = e.currentTarget.getAttribute("data-topic-id");
          const subscribeBtn = e.currentTarget.querySelector(
            ".topic-suggestion-subscribe-btn"
          );
          const isSubscribed =
            subscribeBtn && subscribeBtn.textContent.trim() === "★";

          if (!isSubscribed) {
            // Show a message that they need to subscribe first
            this.showToast(
              "Please subscribe to this topic to view details",
              "info"
            );
            return;
          }

          const topicName = e.currentTarget.querySelector(
    ...
    suggestionsContainer
      .querySelectorAll(".topic-suggestion-subscribe-btn")
      .forEach((btn) => {
  ...
  async showPartnerDetails(keywordId) {
    try {
      ...
      const keywordDetails = await this.apiCall(
        `/extension/horizon/keywords/${keywordId}/details`
      );
      ...
      ).textContent = `${keywordName}`;

      // Show meta summary if available
      const metaSummaryContainer = document.getElementById(
        "partner-screen-meta-summary"
      );
      const hasRealSummary =
        keywordDetails.meta_summary &&
        !keywordDetails.meta_summary.includes("No meta summary available");

      if (hasRealSummary) {
        metaSummaryContainer.innerHTML = this.formatMetaSummary(
          keywordDetails.meta_summary
        );
      } else {
      ...
      if (
        keywordDetails.feed_entries &&
        keywordDetails.feed_entries.length > 0
      ) {
        this.renderPartnerScreenFeedEntries(
          keywordDetails.feed_entries,
          feedListContainer
        );

        // Use summary level sentiment from stored partner data
        const partnerData = this.allPartners?.find(
          (p) => p.keyword_id === keywordId
        );
        ...
        (entry) => `
          ...
          entry.title
                    ...
                    <div class="partnerwatch-tile-actions">
                        <button class="partnerwatch-action-btn" data-action="share_summary" data-entry-id="${
                          entry.entry_id
                        }" data-entry-url="${this.escapeHtml(
          entry.link
        )}" data-entry-title="${this.escapeHtml(
          ...
          entry.five_point_summary || ""
                ...
                <div class="partnerwatch-tile-summary">${this.escapeHtml(
                  entry.one_line_summary ||
                    entry.content_preview ||
                    "No summary available"
                )}</div>
                ${
                  !this.isEpochDate(entry.published_date)
                    ? `<div class="partnerwatch-tile-date">Published on: ${this.formatActualDate(
                        entry.published_date
                      )}</div>`
                    : ""
                }
  ...
  showPartnerSentimentIndicator(sentiment) {
    const sentimentIndicator = document.getElementById(
      "partner-sentiment-indicator"
    );
  ...
  handleEditPartnerFromScreen() {
    ...
    if (keywordId && keywordName) {
      ...
      const partnerTile = document.querySelector(
        `[data-keyword-id="${keywordId}"]`
      );
      const frequency = partnerTile
        ? partnerTile.getAttribute("data-keyword-frequency") || "daily"
        : "daily";
  ...
  async handleSavePartner() {
    ...
    try {
      ...
      if (result && result.id) {
        console.log(
          "Attempting to trigger workflow for keyword_id:",
          result.id
        );
        try {
  ...
  async copyTopicSummary() {
    const title = document.getElementById(
      "overall-topic-summary-title"
    ).textContent;
    const summary = document.getElementById(
      "overall-topic-summary-text"
    ).textContent;
  ...
  showPartnerSummaryModal() {
    ...
    // Populate modal content
    document.getElementById("overall-partner-summary-title").textContent =
      title;
    document.getElementById("overall-partner-summary-text").textContent =
      summary;
  ...
  async copyPartnerSummary() {
    const title = document.getElementById(
      "overall-partner-summary-title"
    ).textContent;
    const summary = document.getElementById(
      "overall-partner-summary-text"
    ).textContent;
  ...
  async confirmUnsubscribe() {
    ...
    try {
      ...
      this.showToast(
        `Successfully unsubscribed from "${topicName}"!`,
        "success"
      );
  ...
  async copySupportEmail() {
    ...
    try {
      ...
      try {
        ...
        this.showToast(
          "Failed to copy email. Please manually copy: " + supportEmail,
          "error"
        );
      }
    }
  ...
  showEditPartnerModal(keywordId, keywordName, frequency) {
    ...
    document.getElementById("edit-frequency-select").value =
      frequency || "daily";
  ...
  async handleUpdatePartner() {
    const partnerName = document
      .getElementById("edit-partner-input")
      .value.trim();
    ...
    try {
      ...
      const result = await this.apiCall(
        `/horizon/keywords/${this.currentEditingKeywordId}`,
        {
          method: "PUT",
          body: JSON.stringify({
            keyword: partnerName,
  ...
  async deletePartner(keywordId, keywordName) {
    try {
      ...
      this.showToast(
        `Partner "${keywordName}" deleted successfully!`,
        "success"
      );
